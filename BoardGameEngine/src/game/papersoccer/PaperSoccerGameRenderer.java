...
import game.papersoccer.PaperSoccerPositionHistory.UndoPaperSoccerMove;
...
public class PaperSoccerGameRenderer implements IGameRenderer<Integer, PaperSoccerPosition>, DrawingMethods {
	...
	private static final Color P1_COLOR = new Color(85, 85, 255);
	private static final Color FIELD_COLOR = new Color(80, 160, 72);
	private static final Color DOT_COLOR = BoardGameEngineMain.BACKGROUND_COLOR;
	private static final Color BALL_COLOR = BoardGameEngineMain.FOREGROUND_COLOR;

	private BoardSizer sizer;
	private double ballRadius;
	private double halfBallRadius;
	...
	public void initializeAndDrawBoard(Graphics2D g) {
		...
		ballRadius = sizer.cellWidth / 6;
		halfBallRadius = ballRadius / 2;

		fillRect(g, 0, 0, imageWidth, imageHeight, new Color(25, 127, 0));

		fillRect(g, sizer.getCenterX(5), sizer.getCenterY(0), sizer.getCenterX(7) - sizer.getCenterX(5), sizer.getCenterY(1) - sizer.getCenterY(0), FIELD_COLOR);
		fillRect(g, sizer.getCenterX(2), sizer.getCenterY(1), sizer.getCenterX(10) - sizer.getCenterX(2), sizer.getCenterY(11) - sizer.getCenterY(1), FIELD_COLOR);
		fillRect(g, sizer.getCenterX(5), sizer.getCenterY(11), sizer.getCenterX(7) - sizer.getCenterX(5), sizer.getCenterY(12) - sizer.getCenterY(11), FIELD_COLOR);

		g.setColor(DOT_COLOR);
		drawCircle(g, sizer.getCenterX(5), sizer.getCenterY(0), 1);
		drawCircle(g, sizer.getCenterX(6), sizer.getCenterY(0), 1);
		drawCircle(g, sizer.getCenterX(7), sizer.getCenterY(0), 1);
		drawCircle(g, sizer.getCenterX(5), sizer.getCenterY(12), 1);
		drawCircle(g, sizer.getCenterX(6), sizer.getCenterY(12), 1);
		drawCircle(g, sizer.getCenterX(7), sizer.getCenterY(12), 1);
		for (int x = 2; x < PaperSoccerUtilities.BOARD_WIDTH - 2; ++x) {
			for (int y = 1; y < PaperSoccerUtilities.BOARD_WIDTH - 1; ++y) {
				drawCircle(g, sizer.getCenterX(x), sizer.getCenterY(y), 1);
			}
		}
		...
		for (int i = 0; i < PaperSoccerUtilities.BOARD_SIZE; ++i) {
			if (INITIAL_POSITION.board[i] != 0) {
				...
				drawPaths(g, i, INITIAL_POSITION.board[i], true);
			}
		}
	}

	private void drawPaths(Graphics2D g, int location, int value, boolean taken) {
		...
		for (int i = 0; i < directionsTaken.length; ++i) {
			...
			drawThickLine(g, x0, y0, x1, y1, 3, true);
		}
	}
	...
	private void drawMoves(Graphics2D g, PaperSoccerPosition position) {
		...
		while (i > 0) {
			...
			drawThickLine(g, sizer.getCenterX(fromCoord.x), sizer.getCenterY(fromCoord.y), sizer.getCenterX(toCoord.x), sizer.getCenterY(toCoord.y), 3, true);
			toCoord = fromCoord;
			--i;
		}
	}
	...
	private void drawBall(Graphics2D g, PaperSoccerPosition position) {
		if (!position.gameOver) {
			g.setColor(FIELD_COLOR.brighter());
			drawPaths(g, position.ballLocation, position.board[position.ballLocation], false);
		}
		g.setColor(BALL_COLOR);
		...
		fillCircle(g, sizer.getCenterX(ballCoordinate.x) + halfBallRadius, sizer.getCenterY(ballCoordinate.y), halfBallRadius / 2);
		fillCircle(g, sizer.getCenterX(ballCoordinate.x) - halfBallRadius, sizer.getCenterY(ballCoordinate.y), halfBallRadius / 2);
		fillCircle(g, sizer.getCenterX(ballCoordinate.x), sizer.getCenterY(ballCoordinate.y) + halfBallRadius, halfBallRadius / 2);
		fillCircle(g, sizer.getCenterX(ballCoordinate.x), sizer.getCenterY(ballCoordinate.y) - halfBallRadius, halfBallRadius / 2);
	}
