...
public class GameTreeSearchTest {
    private static <M, P extends IPosition<M>> GameTreeSearch<M> newGameTreeSearch(P position, int plies, int maxMoves,
        ...
        return new GameTreeSearch<>(strategy.newForkableSearch(null, position, moveList, moveListFactory, plies),
                (canceled, moveWithResult) -> results.add(moveWithResult.result));
    }

    @Test
    public void testSearch() {
        List<AnalysisResult<TestGameNode>> results = new ArrayList<>();
        GameTreeSearch<TestGameNode> treeSearch = newGameTreeSearch(TestGamePosition.createTestPosition(), 1, 2, new TestGameEvaluator(), results);
        treeSearch.search();
    ...
    public void testFork_NotRunning() {
        ...
        GameTreeSearch<TestLockingNode> treeSearch = newGameTreeSearch(testUnlockedLockingPosition, 1, 3, new TestLockingEvaluator(), results);

        List<GameTreeSearch<TestLockingNode>> trees = treeSearch.fork();

        for (GameTreeSearch<TestLockingNode> subTreeSearch : trees) {
    ...
    public void testFork() {
        ...
        List<AnalysisResult<TestLockingNode>> results = new ArrayList<>();
        GameTreeSearch<TestLockingNode> treeSearch = newGameTreeSearch(testUnlockedLockingPosition, 1, 2, new TestLockingEvaluator(), results);

        List<GameTreeSearch<TestLockingNode>> treeSearches = treeSearch.fork();
    ...
    public void testFork_Running() throws InterruptedException {
        ...
        List<AnalysisResult<TestLockingNode>> results = new ArrayList<>();
        GameTreeSearch<TestLockingNode> treeSearch = newGameTreeSearch(testLockingPosition, 1, 3, new TestLockingEvaluator(), results);
        ...
        for (GameTreeSearch<TestLockingNode> subTreeSearch : treeSearches) {
            subTreeSearch.search();
        }
    ...
    public void testFork_MultiFork() {
        List<AnalysisResult<TestGameNode>> results = new ArrayList<>();
        GameTreeSearch<TestGameNode> tree = newGameTreeSearch(TestGamePosition.createTestPosition(), 4, 2, new TestGameEvaluator(), results);

        List<GameTreeSearch<TestGameNode>> multiFork = new ArrayList<>();
        for (GameTreeSearch<TestGameNode> subTree : tree.fork()) {
            multiFork.addAll(subTree.fork());
        }

        for (GameTreeSearch<TestGameNode> gameTreeSearch : multiFork) {
    ...
    public void testDoNotIncludeUnfinishedForks() {
        List<AnalysisResult<TestGameNode>> results = new ArrayList<>();
        GameTreeSearch<TestGameNode> tree = newGameTreeSearch(TestGamePosition.createTestPosition(), 3, 2, new TestGameEvaluator(), results);

        List<GameTreeSearch<TestGameNode>> fork = tree.fork();
        List<GameTreeSearch<TestGameNode>> forkPos1 = fork.get(0).fork();
        List<GameTreeSearch<TestGameNode>> forkPos2 = fork.get(1).fork();
        GameTreeSearch<TestGameNode> pos6 = forkPos1.get(0);
        GameTreeSearch<TestGameNode> pos5 = forkPos1.get(1);
        GameTreeSearch<TestGameNode> pos4 = forkPos2.get(0);
        GameTreeSearch<TestGameNode> pos3 = forkPos2.get(1);
        pos6.search();
        pos5.search();
        pos4.search();
        pos3.stopSearch();
    ...
    public void testRootIncludesFinishedForks() {
        ...
        GameTreeSearch<TestGameNode> pos4 = forkPos2.get(0);
        GameTreeSearch<TestGameNode> pos3 = forkPos2.get(1);
        List<GameTreeSearch<TestGameNode>> forkPos5 = forkPos1.get(1).fork();
        GameTreeSearch<TestGameNode> pos9 = forkPos5.get(0);
        GameTreeSearch<TestGameNode> pos10 = forkPos5.get(1);
        pos6.search();
        pos4.search();
        pos3.search();
        pos9.search();
