...
public class GuiMoveHistory<M> implements Drawable {
    ...
    static class MoveMenuItem implements Drawable {
        ...
        @Override
        public void drawOn(Graphics2D graphics) {
            drawCenteredString(graphics, BoardGameEngineMain.DEFAULT_SMALL_FONT, moveString, x0 + MOVE_WIDTH / 2,
                    y0 + BoardGameEngineMain.DEFAULT_SMALL_FONT_HEIGHT / 2);
        }
        ...
        boolean checkContainsCursor(MouseTracker mouseTracker) {
            return mouseTracker.isMouseEntered && mouseTracker.mouseY >= y0 && mouseTracker.mouseY <= y1 && mouseTracker.mouseX >= x0
                    && mouseTracker.mouseX <= x1;
        }
    }
}
